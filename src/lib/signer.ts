...
class SignerManager {
  ...
  getSigner(): ethers.Wallet {
    ...
    const privateKey = isProduction ? env.SIGNER_PRIVATE_KEY_PROD : env.SIGNER_PRIVATE_KEY_DEV;
  ...
  getSignerAddress(): string {
    const isProduction = env.NODE_ENV === 'production';
    return isProduction ? process.env.NEXT_PUBLIC_SIGNER_ADDRESS_PROD! : process.env.NEXT_PUBLIC_SIGNER_ADDRESS_DEV!;
  }
...
export const signMessage = (userAddress: string, nonce: string) => signerManager.signMessage(userAddress, nonce);
